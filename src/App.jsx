  ...
  return (
    <div style={{ paddingBottom: 80 }}>
      ...
      <div style={{ display: 'flex', gap: 8, marginBottom: 12 }}>
        {[
          { key: 'tarjeta', icon: '💳', label: 'Tarjeta', total: totalGastoTarjeta },
          { key: 'transferencia', icon: '🏦', label: 'Transferencia', total: totalGastoTransferencia },
        ].map((p) => (
          <button
            key={p.key}
            onClick={() => setPagoView((v) => (v === p.key ? null : p.key))}
            style={{
              flex: 1,
              display: 'flex',
              flexDirection: 'column',
              alignItems: 'center',
              gap: 2,
              background: pagoView === p.key ? P.ac : P.cd,
              border: pagoView === p.key ? 'none' : `1px solid ${P.bd}`,
              borderRadius: 10,
              padding: '9px 4px',
              cursor: 'pointer',
            }}
          >
            <span style={{ fontSize: 12, fontWeight: 600, color: pagoView === p.key ? '#fff' : P.tx }}>{p.icon} {p.label}</span>
            <span style={{ fontSize: 11, fontWeight: 700, color: pagoView === p.key ? 'rgba(255,255,255,.9)' : P.sb }}>{fmtS(p.total, cur)}</span>
          </button>
        ))}
      </div>
...
function HomeTab({
  ...
  const distSegments = (() => {
    ...
    const suscAmt = gastos.filter((t) => isSuscHome(t)).reduce((s, t) => s + t.amt, 0);
    const recAmt = gastos.filter((t) => t.recurring && !isSuscHome(t)).reduce((s, t) => s + t.amt, 0);
    const cuotaAmt = gastos.filter((t) => !t.recurring && !isSuscHome(t) && t.pay === 'credito' && t.cuotas > 1).reduce((s, t) => s + t.amt, 0);
    const otherAmt = gastos.filter((t) => !t.recurring && !isSuscHome(t) && !(t.pay === 'credito' && t.cuotas > 1)).reduce((s, t) => s + t.amt, 0);
          ...
          <Box>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
              {byCat.slice(0, 5).map(([catName, amt]) => {
                ...
                const subs = open ? subsOf(catName) : [];
                return (
                  <div key={catName}>
                    <div onClick={() => setExpandedCat(open ? null : catName)} style={{ display: 'flex', alignItems: 'center', gap: 12, cursor: 'pointer' }}>
